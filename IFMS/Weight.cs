...
namespace IFMS
{
    public partial class Weight : Form
    {
        ...
        public void show()
        {
            try
            {
                queryClass.selectds("Select * from Weight");
                dgvWeight.DataSource = queryClass.ds.Tables[0];
            }
            catch (Exception e)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                string qry = string.Format("Insert into Weight(Vehicle_Id,Net_Weight,Tare,Deduction,Date_Time,Price)values('" + tbVehicle_Id.Text + "','" + tbNet_Weight.Text + "','" + tbTare.Text + "','" + tbDeduction.Text + "','" + dtDate_Time.Value + "','" + tbPrice.Text + "')");
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (iExit == DialogResult.Yes)
                    {
                        ...
                        string qry = string.Format("update Weight set Vehicle_Id='" + tbVehicle_Id.Text + "',Net_Weight='" + tbNet_Weight.Text + "',Tare='" + tbTare.Text + "',Deduction='" + tbDeduction.Text + "',Date_Time='" + dtDate_Time.Value + "',Price='" + tbPrice.Text + "' where ID=@i");
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (iExit == DialogResult.Yes)
                    {
                        string qry = string.Format("delete from Weight where ID=@i");
        ...
        private void dgvWeight_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                queryClass.id = Convert.ToInt32(dgvWeight.Rows[e.RowIndex].Cells[0].Value.ToString());
            }
            catch (Exception)
            {
        ...
        private void dgvWeight_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                tbVehicle_Id.Text = dgvWeight.Rows[e.RowIndex].Cells[1].Value.ToString();
                tbNet_Weight.Text = dgvWeight.Rows[e.RowIndex].Cells[2].Value.ToString();
                tbTare.Text = dgvWeight.Rows[e.RowIndex].Cells[3].Value.ToString();
                tbDeduction.Text = dgvWeight.Rows[e.RowIndex].Cells[4].Value.ToString();
                dtDate_Time.Text = dgvWeight.Rows[e.RowIndex].Cells[5].Value.ToString();
                tbPrice.Text = dgvWeight.Rows[e.RowIndex].Cells[6].Value.ToString();
            }
            catch (Exception x)
            {
                MessageBox.Show(x.Message);
        ...
        private void tbTare_KeyPress(object sender, KeyPressEventArgs e)
        {
            queryClass.valNumDec(e);
        }
